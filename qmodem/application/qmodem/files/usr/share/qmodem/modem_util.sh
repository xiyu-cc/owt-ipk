...
at()
{
  ...
  local options_local="$options"
  [ "$clear_buffer" == "1" ] && options_local="$options_local -M"
  if [ -n "$timeout" ]; then
    options_local="$options_local -t $timeout"
    case "$atcmd" in
      *QSCAN*) options_local="$options_local -g" ;;
    esac
  fi
  #过滤空行
  if [ "$(uci get qmodem.main.at_tool 2>/dev/null)" == "1" ]; then
   if [ -n "$timeout" ]; then
    sms_tool_q -t "$timeout" -d $at_port at "$atcmd"
   else
    sms_tool_q -d $at_port at "$atcmd"
   fi
  else
   tom_modem $use_ubus_flag  -d $at_port -o a -c "$atcmd" $options_local
  fi
}
