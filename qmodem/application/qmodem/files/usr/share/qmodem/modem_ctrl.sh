...
[ -n "$vendor" ] && vendor=$(printf '%s' "$vendor" | tr 'A-Z' 'a-z' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
[ -n "$platform" ] && platform=$(printf '%s' "$platform" | tr 'A-Z' 'a-z' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
case $method in
    ...
    "get_neighborcell")
        if [ -n "$3" ]; then
            neighborcell_async=$(echo "$3" | jq -r '.async // 0')
            neighborcell_force=$(echo "$3" | jq -r '.force // 0')
            neighborcell_timeout=$(echo "$3" | jq -r '.timeout // empty')
            neighborcell_scan_mode=$(echo "$3" | jq -r '.scan_mode // empty')
        else
            neighborcell_async="0"
            neighborcell_force="0"
            neighborcell_timeout=""
            neighborcell_scan_mode=""
        fi
        get_neighborcell
        ;;
    "get_network_prefer")
