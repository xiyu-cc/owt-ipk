...
get_neighborcell_qualcomm(){
    local scan_mode="${neighborcell_scan_mode:-2}"
    case "$scan_mode" in
        1|2) ;;
        *) scan_mode=2 ;;
    esac
    local at_command="AT+QSCAN=${scan_mode},1"
    local scan_timeout="${neighborcell_timeout:-300}"
    local scan_prefix="/tmp/neighborcell_${config_section}"
    local scan_tmp="${scan_prefix}.tmp"
    local scan_out="${scan_prefix}.out"
    local scan_pid="${scan_prefix}.pid"
    local scan_status="done"
    local scan_source=""
    local async="${neighborcell_async:-0}"
    local force="${neighborcell_force:-0}"
    local lock_timeout=5
    local lte_status=""
    local nr_status=""
    local lte_lock_status=""
    local lte_lock_freq=""
    local lte_lock_pci=""
    local nr_lock_status=""
    local nr_lock_pci=""
    local nr_lock_freq=""
    local nr_lock_scs=""
    local nr_lock_band=""
    ...
    scan_pid_running() {
        local pid
        [ -f "$scan_pid" ] || return 1
        pid=$(cat "$scan_pid" 2>/dev/null)
        [ -n "$pid" ] || { rm -f "$scan_pid"; return 1; }
        kill -0 "$pid" 2>/dev/null || { rm -f "$scan_pid"; return 1; }
        return 0
    }

    if [ "$async" = "1" ]; then
        if [ "$force" = "1" ] && ! scan_pid_running; then
            rm -f "$scan_out" "$scan_tmp" "${scan_prefix}.time"
            (
                at $at_port $at_command $scan_timeout > "$scan_tmp"
                mv -f "$scan_tmp" "$scan_out"
                rm -f "$scan_pid"
                date +%s > "${scan_prefix}.time"
            ) &
            echo $! > "$scan_pid"
        fi
        if scan_pid_running; then
            scan_status="scanning"
        else
            scan_status="done"
        fi
    else
        at $at_port $at_command $scan_timeout > "$scan_out"
    fi
    if [ "$scan_status" = "done" ]; then
        scan_source="$scan_out"
    fi

    if [ "$scan_status" = "done" ]; then
        nr_lock_check="AT+QNWLOCK=\"common/5g\""
        lte_lock_check="AT+QNWLOCK=\"common/4g\""
        lte_status=$(at $at_port $lte_lock_check $lock_timeout | grep "+QNWLOCK:")
        if [ -n "$lte_status" ]; then
            ...
            if [ -n "$lte_lock_status" ] && [ "$lte_lock_status" != "0" ]; then
                lte_lock_status="locked"
            else
                lte_lock_status=""
            fi
        fi
        nr_status=$(at $at_port $nr_lock_check $lock_timeout | grep "+QNWLOCK:")
        if [ -n "$nr_status" ]; then
            nr_lock_status=""
            nr_lock_pci=""
            nr_lock_freq=""
            nr_lock_scs=""
            nr_lock_band=""
            nr_field_count=$(echo "$nr_status" | awk -F',' '{print NF}')
            if [ "$nr_field_count" -ge 6 ]; then
                nr_lock_mode=$(echo "$nr_status" | awk -F',' '{print $2}' | sed 's/\r//g')
                nr_lock_freq=$(echo "$nr_status" | awk -F',' '{print $3}' | sed 's/\r//g')
                nr_lock_pci=$(echo "$nr_status" | awk -F',' '{print $4}' | sed 's/\r//g')
                nr_lock_scs=$(echo "$nr_status" | awk -F',' '{print $5}' | sed 's/\r//g')
                nr_lock_band=$(echo "$nr_status" | awk -F',' '{print $6}' | sed 's/\r//g')
                if [ -n "$nr_lock_mode" ] && [ "$nr_lock_mode" != "0" ]; then
                ...
                if [ -n "$nr_lock_pci" ] && [ "$nr_lock_pci" != "0" ] && [ -n "$nr_lock_freq" ] && [ "$nr_lock_freq" != "0" ]; then
                    nr_lock_status="locked"
                fi
            fi
        fi
    ...
    json_close_object
    json_add_array "NR"
    if [ -n "$scan_source" ] && [ -f "$scan_source" ]; then
        while read -r line; do
            if [ -n "$(echo $line | grep "+QSCAN:")" ]; then
                line=$(echo "$line" | sed 's/^.*+QSCAN: //g' | tr -d '\r')
                rat=$(echo "$line" | awk -F',' '{print $1}' | tr -d '"' | xargs)
                case "$rat" in
                    NR5G*) ;;
                    *) continue ;;
                esac
                mode=""
                rat_upper=$(echo "$rat" | tr '[:lower:]' '[:upper:]')
                case "$rat_upper" in
                    *NSA*) mode="NSA" ;;
                    *SA*) mode="SA" ;;
                esac
                ...
                bandwidth=""
                band=""
                mcc=$(echo "$line" | awk -F',' '{print $2}' | tr -d '"' | xargs)
                mnc=$(echo "$line" | awk -F',' '{print $3}' | tr -d '"' | xargs)
                arfcn=$(echo "$line" | awk -F',' '{print $4}' | xargs)
                pci=$(echo "$line" | awk -F',' '{print $5}' | xargs)
                rsrp=$(echo "$line" | awk -F',' '{print $6}' | xargs)
                rsrq=$(echo "$line" | awk -F',' '{print $7}' | xargs)
                srxlev=$(echo "$line" | awk -F',' '{print $8}' | xargs)
                scs=$(echo "$line" | awk -F',' '{print $9}' | xargs)
                cellid=$(echo "$line" | awk -F',' '{print $10}' | tr -d '"' | xargs)
                tac=$(echo "$line" | awk -F',' '{print $11}' | tr -d '"' | xargs)
                bandwidth=$(echo "$line" | awk -F',' '{print $12}' | xargs)
                band=$(echo "$line" | awk -F',' '{print $13}' | xargs)
                json_add_object ""
    ...
    if [ -n "$scan_source" ] && [ -f "$scan_source" ]; then
        while read -r line; do
            if [ -n "$(echo $line | grep "+QSCAN:")" ]; then
                ...
                rsrq=$(echo "$line" | awk -F',' '{print $7}' | xargs)
                srxlev=$(echo "$line" | awk -F',' '{print $8}' | xargs)
                squal=$(echo "$line" | awk -F',' '{print $9}' | xargs)
                cellid=$(echo "$line" | awk -F',' '{print $10}' | tr -d '"' | xargs)
                tac=$(echo "$line" | awk -F',' '{print $11}' | tr -d '"' | xargs)
                json_add_object ""
                json_add_string "mcc" "$mcc"
    ...
    if [ "$scan_status" = "scanning" ]; then
        json_add_string "LTE" "scanning"
        json_add_string "NR" "scanning"
    else
    ...
    json_close_object
    json_add_string "scan_status" "$scan_status"
}
...
lockpci_nr(){
    ...
    if [ "$cell_type" = "0" ]; then
        ...
        locknr="AT+QNWLOCK=\"common/5g\",1,$arfcn,$pci,$scs,$nrband"
        echo $locknr
        res=$(at $at_port $locknr)
    fi
}
